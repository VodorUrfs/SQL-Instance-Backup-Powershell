...
foreach ($instance in $service_name) 
{
    ...
    if ($item.CreationTime -lt (date).addminutes($Daysback)) { } # Makes sure our most recent backup isnt more than $daysback old if it is nothing gets done.
    else { 
            # Counts files being deleted deleted that are older than $daysback
            $filestodelete=(Get-ChildItem $path\$instancefolder | Where-Object { $_.LastWriteTime -lt (date).addminutes($Daysback) })
            $filesdeleted = $filestodelete.Count
    
            # Deletes the files that are older than $daysback
            Get-ChildItem $path\$instancefolder | Where-Object { $_.LastWriteTime -lt (date).addminutes($Daysback) } | Remove-Item 
